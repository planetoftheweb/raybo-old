...
summary: In order to work with data-visualization, it's important to understand the different formats that you can work with in order to create data. In this module, I'll discuss different formats for storing data, their advantages and disadvantages.
...
Notice how beautiful that is to read and if you were to give this to someone to type in a text editor, it would be fairly easy for even someone without YAML experience to stick to the rules.  Here's what that would translate to as JSON. Unlike with JSON, strings do not have to be quoted. Also, notice that although the whitespace is relevant, in YAML, we're adding tabs to make things align an make them more readable. Here's what that data would look like expressed in JSON.
